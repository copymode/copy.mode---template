...
export const AgentAvatarUploader = ({ 
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log("1. Iniciando upload de avatar:", { file, agentId });
    ...
    try {
      ...
      // Gerar um ID temporário se ainda não tiver um agentId
      const id = agentId || `temp-${uuidv4()}`;
      
      // Gerar um caminho único para o avatar do agente
      const filePath = `${id}/${Date.now()}-${file.name}`;
      console.log("4. Tentando upload para bucket 'agent-avatars', caminho:", filePath);
        ...
        try {
          ...
          try {
            uploadResult = await supabase.storage
              ...
              .upload(`agents/${id}/${Date.now()}-${file.name}`, file, {
                upsert: true,
              });
      ...
      const bucketName = uploadResult.data.fullPath ? uploadResult.data.fullPath.split('/')[0] : 'agent-avatars';
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Ocorreu um erro ao atualizar a foto do agente.",
        variant: "destructive",
      });
    } finally {
      setIsUploadingAvatar(false);
  ...
  return (
    <div className="relative">
      <Avatar className={sizeClasses[size]}>
        {avatarUrl ? (
          <AvatarImage src={avatarUrl} alt={agentName || "Avatar"} />
        ) : (
      ...
      <Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Atualizar foto do agente</DialogTitle>
          </DialogHeader>
